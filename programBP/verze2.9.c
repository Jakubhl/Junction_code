...
if(refresh_ready==false){  
  ...
  if (refresh != false && priority == 0 && refresh_ready==true && inputs.idle==0) { //idle==0 kvůli přechodu, který může přetrvat do idle stavu a spustí refresh (nechceme)
...
//vypisování pro emergency stav---------------------------------------------------------------------------------------
  ...
  if (inputs.emergency == 1 && priority == 0) {
    ...
    if (trlc.traffic_light != 1 && trlc.traffic_light != 4 && trlc.traffic_light != 8 && inputs.idle == 0 && disp_no_dir == false) {
      lcd.setCursor ( 0, 1 );
  ...
  switch (loading)
  {
    case 1:
      ...
      if (inputs.idle == 0 && stop_loading == false && priority == 0) {
...
//odpocitavani na displeji jen pokud neprobihaji priority ani necinny stav
  if (inputs.idle == 0 && priority == 0 && stop_loading==false) {
    if (sec2 != sec) {
...
if(inputs.cross_reg_not_repeat!=0 && trlc.cross_reg_not_repeat==1){
  ...
  //nejprve se navaze, potom se prispusobuji led tak aby byl prujezdny stav pouze v jednom smeru
    ...
    if(trlc.reset_priority==true){
      clck_down = 1; clck_up = 0; disp_priority = false; mins = 0; min_pos = 10; sec_pos2 = 10;
